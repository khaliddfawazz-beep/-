...
export default function Opportunities() {
  ...
  const handleJoinOpportunity = (opportunity) => {
    setJoinedOpportunities([...joinedOpportunities, opportunity]);
    ...
    <div className="min-h-screen bg-slate-100 p-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {opportunities.map((item, index) => {
          const alreadyJoined = joinedOpportunities.some(
            (joined) => joined.title === item.title
          );
            ...
            >
              ...
              <div className="p-6 text-right space-y-4">
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-2xl transition font-semibold text-white ${
                    alreadyJoined
                      ? "bg-slate-400 cursor-not-allowed"
                      : "bg-green-700 hover:bg-green-800"
                  }`}
                >
                  {alreadyJoined ? "تم الانضمام" : "انضم الآن"}
                </button>
              </div>
            </div>
          );
